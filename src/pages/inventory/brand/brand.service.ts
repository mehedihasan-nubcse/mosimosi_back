...
import { Brand } from 'src/interfaces/common/brand.interface';
...
export class BrandService {
  ...
  async addBrandByShop(
    shop: string,
    addColorDto: AddColorDto,
  ): Promise<ResponsePayload> {
    try {
      const createdAtString = this.utilsService.getDateString(new Date());
      const data = new this.brandModel({
        ...addColorDto,
        createdAtString,
        shop: shop,
      });
  ...
  async getAllBrandByShop(
    ...
    try {
      ...
      return this.getAllBrands(filterColorDto, searchQuery);
    } catch (error) {
      console.log(error);
      throw new InternalServerErrorException(error.message);
    }
