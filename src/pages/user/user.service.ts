...
export class UserService {
  ...
  async checkUserForRegistration(
    ...
    try {
      ...
      if (userData) {
        ...
        return {
          success: false,
          message: 'UserInterface not exists. Please check your phone number',
          data: { otp: false },
        } as ResponsePayload;
      }
    } catch (error) {
  ...
  /**
   * Get UserInterface by ID
   * Update UserInterface by Id
   * Delete UserInterface by Id
   */
  ...
  async updateLoggedInUserInfo(
    ...
    if (!user) {
      throw new NotFoundException('No UserInterface found!');
    }
    try {
      // Check Username
      if (username) {
  ...
  async checkUserAndSentOtp(
    ...
    try {
      ...
      return {
        success: false,
        message: 'UserInterface no exists',
      } as ResponsePayload;
