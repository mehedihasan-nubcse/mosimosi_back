...
export class AdminService {
  ...
  async adminSignup(createAdminDto: CreateAdminDto): Promise<ResponsePayload> {
    const { password, shops } = createAdminDto;
  ...
  async adminLogin(authAdminDto: AuthAdminDto): Promise<AdminAuthResponse> {
    try {
      const user = (await this.adminModel
        ...
        .select('password username role permissions hasAccess shops')) as Admin;
      ...
      if (isMatch) {
        ...
        // Find Shops
        const adminShops = user.shops.map((m) => new ObjectId(m));
        ...
        if (!jShops.length) {
          return {
            success: false,
            message: `Sorry! You have no shop for access`,
          } as AdminAuthResponse;
        }
        ...
        const mShops = jShops.map((m: Shop) => {
          return {
            ...
            phoneNo: m.phoneNo,
            address: m.address,
            user: m.users.find((f) => f._id.toString() === user._id.toString()),
          };
        });
  ...
  async getAllAdmins(
    ...
    if (filter) {
      if (filter['shops._id']) {
        filter['shops._id'] = new ObjectId(filter['shops._id']);
      }
    }
  ...
  async getAllUserByShop(
    admin: Admin,
    shop: string,
    filterAndPaginationAdminDto: FilterAndPaginationAdminDto,
    searchQuery?: string,
  ): Promise<ResponsePayload> {
    try {
      if (!shop) {
      ...
      // Modify Filter
      const { filter } = filterAndPaginationAdminDto;
      filterAndPaginationAdminDto.filter = {
        ...filter,
        ...{ 'shops._id': shop },
      };

      return this.getAllAdmins(filterAndPaginationAdminDto, searchQuery);
    } catch (error) {
      console.log(error);
      throw new InternalServerErrorException(error.message);
    }
