...
export class ShopInformationService {
  ...
  async addShopInformation(
    ...
    const newData = new this.shopInformationModel({
      ...addShopInformationDto,
      ...{
        shop: shop,
      },
  ...
  async getAllShopInformationByShop(
    shop: string,
    filterShopInformationDto: FilterAndPaginationShopInformationDto,
    searchQuery?: string,
  ): Promise<ResponsePayload> {
    try {
      ...
      // Modify Filter
      const { filter } = filterShopInformationDto;
      filterShopInformationDto.filter = { ...filter, ...{ shop: shop } };

      return this.getAllShopInformations(filterShopInformationDto, searchQuery);
    } catch (error) {
      console.log(error);
      throw new InternalServerErrorException(error.message);
    }
  ...
  async getShopInformation(
    ...
    try {
      // const data = await this.shopInformationModel.findOne({shop:shop}).select(select);
      const data = await this.shopInformationModel.findOne({ shop: shop });
      return {
        success: true,
        message: 'Success',
        data,
  ...
  async getShopInformationById(
    ...
    try {
      const data = await this.shopInformationModel.findById(id);
      return {
        success: true,
        message: 'Success',
        data,
