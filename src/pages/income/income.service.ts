...
import { Income } from 'src/interfaces/common/income.interface';
...
export class IncomeService {
  ...
  constructor(
    @InjectModel('Income') private readonly incomeModel: Model<Income>,
  ...
  async addIncome( shop: string,addIncomeDto: AddIncomeDto): Promise<ResponsePayload> {
    try {
      const { date } = addIncomeDto;
      ...
      const data = new this.incomeModel(mData);
  ...
  async getAllIncomes(
    ...
    if (searchQuery) {
      mFilter = { ...mFilter, ...{ incomeFor: new RegExp(searchQuery, 'i') } };
    }
    ...
    try {
      const dataAggregates = await this.incomeModel.aggregate(aggregateStages);
      const calculateAggregates = await this.incomeModel.aggregate(
  ...
  async getIncomeById(id: string, select: string): Promise<ResponsePayload> {
    try {
      const data = await this.incomeModel.findById(id);
      return {
        success: true,
        message: 'Success',
        data,
  ...
  async getAllIncomeByShop(
    ...
    try {
      ...
      // Modify Filter
      const { filter } = filterIncomeDto;
      filterIncomeDto.filter = { ...filter, ...{ shop: shop } };

      return this.getAllIncomes(filterIncomeDto, searchQuery);
    } catch (error) {
      console.log(error);
      throw new InternalServerErrorException(error.message);
    }
  ...
  async getIncomeByDate(
    ...
    try {
      const data = await this.incomeModel.find({ date: date });
      return {
        success: true,
        message: 'Success',
        data,
